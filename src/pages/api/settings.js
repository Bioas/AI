...
export default async function handler(req, res) {
  ...
  catch (e) { return res.status(200).json(defaults); }

  const db = client.db('dorm_billing');
  const collection = db.collection('settings');

  if (req.method === 'GET') {
    const s = await collection.findOne({ _id: 'default' });
    return res.status(200).json(s || defaults);
  }
  if (req.method === 'POST') {
    await collection.updateOne({ _id: 'default' }, { $set: req.body }, { upsert: true });
