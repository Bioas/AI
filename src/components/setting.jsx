    ...
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
      ...
      <div className="space-y-5">
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-start justify-between mb-4">
              <h3 className="text-sm font-semibold text-slate-800">🏢 ข้อมูลหอพัก</h3>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-start justify-between mb-4">
              <h3 className="text-sm font-semibold text-slate-800">🖼️ โลโก้หอพัก</h3>
              <Button size="sm" variant="secondary" onClick={() => setActiveSection('logo')}>จัดการ</Button>
            </div>
            <div className="flex items-center gap-4">
              {settings.logo ? (
                <img src={settings.logo} alt="โลโก้" className="w-16 h-16 object-contain rounded-xl border border-slate-200 shadow-sm" />
              ) : (
                <div className="w-16 h-16 rounded-xl bg-gradient-to-br from-slate-100 to-slate-200 flex items-center justify-center text-2xl">🏠</div>
              )}
              <div className="text-xs text-slate-500">{settings.logo ? 'มีโลโก้' : 'ยังไม่มีโลโก้'}</div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-start justify-between mb-4">
              <h3 className="text-sm font-semibold text-slate-800">💰 อัตราค่าใช้จ่าย</h3>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-start justify-between mb-4">
              <div>
                <h3 className="text-sm font-semibold text-slate-800">📱 LINE Messaging</h3>
                <p className="text-xs text-slate-400 mt-0.5">ส่งใบแจ้งหนี้ผ่าน LINE ให้ผู้พัก</p>
              </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-slate-500">{settings.channelToken ? 'เชื่อมต่อ LINE แล้ว' : 'ยังไม่ได้ตั้งค่า LINE Token'}</span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="pt-6">
            <h3 className="text-sm font-semibold text-slate-800 mb-1">🗂️ สำรองและกู้คืนข้อมูล</h3>
            <p className="text-xs text-slate-400 mb-5">ส่งออกข้อมูลเพื่อสำรอง หรือนำเข้าจากไฟล์สำรอง</p>
